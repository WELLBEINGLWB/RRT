...
Draw::Draw(MotionPlan::RRT &rrt) { CreatePotentialField(rrt); }
...
// f(x,y)
double Draw::f_xy(double x,double y)
{
  double sum = 0.0;
...
void Draw::drowSpline(std::vector<POINT> &finalpath)
{
  ...
  for (t = 0; t <= m; t += 0.01) {
    tmp.x = xs.calc(t); tmp.y = ys.calc(t);
    Potential = f_xy(tmp.x, tmp.y);  // その座標のポテンシャル計算
    PotentialSum += Potential;               // ポテンシャルの合計を計算

    if (Potential > MaxPotential) {  // 経路中でもっとも高いポテンシャルを計算（最大値計算）
      MaxPotential = Potential;
      MaxPoint.x = tmp.x; MaxPoint.y = tmp.y;
    }

    SplinePoint.push_back(tmp);
    outStream << tmp.x << "\t" << tmp.y << std::endl;
  ...
  cout << "そのときの座標は = (" << MaxPoint.x << ", " << MaxPoint.y << ")" << endl;
